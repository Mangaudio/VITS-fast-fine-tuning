...
if __name__ == "__main__":
    ...
    assert (
        len(speakers) != 0
    ), "No audio file found. Please check your uploaded file structure."
    ...
    if args.add_auxiliary_data:
        ...
        with open(
            "./configs/modified_finetune_speaker.json", "w", encoding="utf-8"
        ) as f:
        ...
        for i, line in enumerate(new_annos):
            ...
            if len(txt) > 150:
                continue
            cleaned_text = text._clean_text(txt, hps["data"]["text_cleaners"]).replace(
                "[ZH]", ""
            )
            cleaned_text += "\n" if not cleaned_text.endswith("\n") else ""
            cleaned_new_annos.append(
