...
def latest_checkpoint_path(dir_path, regex="G_[0-9]*.pth"):
    ...
    x = f_list[-1]
    logger.info(f"latest_checkpoint_path: {x}")
    return x
...
def get_hparams(init=True):
    ...
    parser.add_argument(
        "-m", "--model", type=str, default="pretrained_models", help="Model name"
    )
    parser.add_argument(
        "-n", "--max_epochs", type=int, default=50, help="finetune epochs"
    )
    parser.add_argument(
        "--cont",
        type=str2bool,
    ...
    parser.add_argument(
        "--preserved", type=int, default=4, help="Number of preserved models"
    )
